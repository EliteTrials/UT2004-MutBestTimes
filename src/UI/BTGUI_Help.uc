...
function InternalOnCategoryChange( GUIComponent sender )
{
    ...
    val = Topics[topicIdx].Contents[0];
	// InfoTextBox.MyScrollText.SetContent( val );
    for( i = 1; i < Topics[topicIdx].Contents.Length; ++ i )
    {
    	val $= "|"$Topics[topicIdx].Contents[i];
    }
	InfoTextBox.MyScrollText.SetContent( val );
...
defaultproperties
{
	begin object class=BTGUI_HelpTopic name=oTopicBasic
		Caption="Basics"
		Contents(0)="ÿPlease read the info here thoroughly before playing!"
	end object
	Topics(0)=oTopicBasic

	begin object class=BTGUI_HelpTopic name=oTopicTrialModes
		Caption="Modes"
		Contents(0)="7tÎSolo BTimes"
		Contents(1)="---------------------|"
		...
		Contents(3)="7tÎRegular BTimes"
		Contents(4)="---------------------|"
		Contents(5)="7tÎGroup BTimes"
		Contents(6)="---------------------|"
	end object
